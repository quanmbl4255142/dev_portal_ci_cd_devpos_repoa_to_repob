...
async def generate_and_deploy(config: AutoDeployConfig):
    ...
    try:
        ...
        # Step 2: Push Django code to Repository_A
        result["steps"].append({"step": "push_to_repo_a", "status": "processing"})
        
        if config.create_new_repo_a:
            repo_result = gh_manager.create_repository_and_push(
        ...
        # Step 2.5: Add PAT_TOKEN secret to Repository_A
        if project_config.enable_cicd and config.create_new_repo_a:
            result["steps"].append({"step": "add_github_secrets", "status": "processing"})
            import time
            time.sleep(2)  # Đợi GitHub xử lý repo
            ...
            result["steps"][-1]["status"] = secret_result["status"]
            result["steps"][-1]["message"] = secret_result["message"]
